...
public class Player {
	...
	public int checkWin(int floor) {
		...
		//searches the array for a suit containing 4 and if its found returns 4
		for (int card : suitTotal) {
			if (card == 4)
				return 4;
		}
		
		//goes though the hand subtracting each card number agains the total to see if it equals 20,
		//and if so returns 20
		for (int i = 0; i < hand.size(); i++) {
			if (total - (hand.get(i) % 10) == 20)
				return 20;
		}
		
		//the base case return is 0 if there are no win conditions met
		return 0;
	}
	...
	public String toString() {
		...
		//starts the stringBuiler with the players name
		player.append(name + "'s hand: ");
		
		//goes through each card number in the hand and gets the appropriate suit and number to add to the StringBuiler.
		for (int i = 0; i < hand.size(); i++) {
			if (hand.get(i) % 10 == 1)
				player.append("Ace");
